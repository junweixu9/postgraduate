...
# =========================
# Test / Attack (WMMD-compatible + optional LA inference)
# =========================
def test(model, device_id, disp_GE=True, model_flag='finetuned',
         apply_logit_adjustment_infer=False, adjustments=None):
    """
    WMMD: no special inference change needed (WMMD is training-time alignment) :contentReference[oaicite:2]{index=2}
    Logit adjustment:
      - default inference does NOT apply LA (common practice) :contentReference[oaicite:3]{index=3}
      - provide optional switch if you want to test both behaviors
    """
    model.eval()
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, label) in enumerate(test_loader):
            ...
            # optional: apply logit adjustment at inference (off by default)
            if apply_logit_adjustment_infer:
                if adjustments is None:
                    raise ValueError("apply_logit_adjustment_infer=True but adjustments is None")
                logits = logits + adjustments  # broadcasting [1,C]

            test_loss += clf_criterion(logits, label).item()

            pred = logits.data.max(1)[1]
            prob = softmax(logits).cpu()

            bs = data.size(0)
            if offset + bs <= test_preds_all.shape[0]:
                test_preds_all[offset:offset + bs, :] = prob
            offset += bs
    ...
    print(f'{domain_name} test loss: {test_loss:.4f}, {domain_name} test accuracy: {correct}/{len(test_loader.dataset)} ({acc:.2f}%)\n')
...
if __name__ == '__main__':
    ...
    # ========== logit adjustment info ==========
    # 训练时您用的是 source profiling 的类别频率；这里测试数据只有 attack，
    # 如果您想“推理阶段也加 adjustment”，我们用 source_attack 的频率近似一下。
    adjust_flag = False
    apply_logit_adjustment_infer = False  # 默认 False（推荐对齐常见做法）
    adjustments = None
    if adjust_flag:
        adjustments_np = compute_adjustment_1(Y_attack_source, tro=1, classes=class_num)
    ...
    # 这里的 ckpt 路径：如果您训练阶段保存的文件名不同（例如包含 wmmd），改这里即可
    ckpt_path = './models/ID{}_wmdd_best_valid_loss_fine_tuned_cpda_device{}_to_{}.pth'.format(
        str(flag), source_device_id, target_device_id
    )
    ...
    with torch.no_grad():
        ...
        test(model, source_device_id, model_flag='finetuned_source',
             apply_logit_adjustment_infer=apply_logit_adjustment_infer,
