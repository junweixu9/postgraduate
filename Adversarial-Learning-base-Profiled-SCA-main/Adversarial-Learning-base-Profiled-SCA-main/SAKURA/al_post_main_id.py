...
def compute_adjustment_1(Y_profiling, tro, classes=256, eps=1e-12):
...
if __name__ == '__main__':
    ...
    if adjust_flag:
        ...
        flag = "fake"
    pretrained_path = ('./models/ID'+str(flag)+'_pre-trained_cpda_device{}.pth'.format(source_device_id))

    device = torch.device('cuda' if cuda else 'cpu')

    model = CDP_Net(num_classes=256).to(device)
    discriminator = Discriminator().to(device)  # ✅ 关键
    # summary(model, input_size=(1, 1000), device='cpu')
    # summary(discriminator, input_size=(1, 1000), device='cpu')
    ...
    for epoch in range(1, finetune_epoch + 1):
        ...
        if (val_total_loss < min_loss):
            ...
            }, './models/alpa_id_'+str(flag)+'final_device{}_to_{}.pth'.format(source_device_id, target_device_id))
            end_time = time.time()
