...
if __name__ == '__main__':
    ...
    lr_manager = OneCycleLR(len(X_profiling[0:5000]), 256, 1e-2, end_percentage=0.2, scale_percentage=0.1, maximum_momentum=None, minimum_momentum=None, verbose=True)

    callback = [
        all(validation=(X_attack[0:5000], plt_attack[0:5000], Y_attack[0:5000])), lr_manager
    ]
    #
    model, batch_size, epoch_sota = DL_model_100.pick_SOAT(dataset, leakage_model, X_profiling.shape[1],
                                                       companion_metric,
                                                       cer_adjustment_loss, learning_rate, model=attack_model,
    ...
    """开始训练"""
    history = model.fit(x=X_profiling[:50000], y=Y_profiling[:50000, :9], batch_size=batch_size, verbose=2,
                        epochs=epoch,
                        callbacks=callback
                        )
    ...
    # Attack
    print('======Attack======')
    predictions = model.predict(X_attack[5000:10000])
    # predictions = predictions - 0.25 * adjustments
    predictions = tf.nn.softmax(predictions)
    attack_traces = perform_attacks(plt_attack[5000:10000], predictions, "attack_traces",
                                    leakage_model, dataset, num_traces_attacks)
    log = open('F:/result/desyn/a/50000_cer_0tro0_desync100_new.txt', mode='a',
               encoding='utf-8')
    print(file=log)
    print(file=log)
