...
def pick_SOAT(data, leakage_model, length, metric, loss, learning_rate, model='MLP', model_size=64):
    if model == 'CNN':
        if data == 'ASCAD' and leakage_model == 'HW':
            batch_size = 256
            epoch = 25
            return ascad_f_hw_cnn(length, metric, loss, learning_rate), batch_size, epoch
...
def ascad_f_hw_cnn_gemini(length, metric, loss, learning_rate):
    img_input = Input(shape=(length, 1), name='input_trace')

    # Block 1: 特征提取
    # 论文中常用 SELU 激活函数和 HeUniform 初始化
    x = Conv1D(filters=8, kernel_size=3, strides=1,
               kernel_initializer='he_uniform', activation='selu', padding='same')(img_input)
