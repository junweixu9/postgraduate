...
import DL_model_50 as DL_model
...
if __name__ == '__main__':
    ...
    lr_manager = OneCycleLR(len(X_attack[0:10000]), 400, 1e-2, end_percentage=0.2, scale_percentage=0.1, maximum_momentum=None, minimum_momentum=None, verbose=True)

    callback = [
        all(validation=(X_attack[0:10000], plt_attack[0:10000], Y_attack[0:10000])), lr_manager
    ]
    #
    ...
    """开始训练"""
    history = model.fit(x=X_profiling[:50000], y=Y_profiling[:50000, :9], batch_size=batch_size, verbose=2, epochs=epoch,
                        callbacks=callback
                        )
    ...
    # Attack
    print('======Attack======')
    predictions = model.predict(X_attack[10000:20000])
    # predictions = predictions - 0.25 * adjustments
    predictions = tf.nn.softmax(predictions)
    attack_traces = perform_attacks(plt_attack[10000:20000], predictions, "attack_traces",
                                    leakage_model, dataset, 20000)
    log = open('F:/result/desyn/ar/50000_ce_0tro1_desync50_new.txt', mode='a',
               encoding='utf-8')
    print(file=log)
    print(file=log)
