...
def compute_adjustment(Y_profiling, Y_attack, tro):
    """compute the base probabilities"""

    Y_profiling = np.argmax(Y_profiling[:, :9], 1)
    Y_attack = np.argmax(Y_attack[:, :9], 1)
...
if __name__ == '__main__':
    ...
    adjustments = compute_adjustment(Y_profiling, Y_attack, tro)
    ...
    lr_manager = OneCycleLR(len(X_attack[:5000]), 50, learning_rate, end_percentage=0.2, scale_percentage=0.1,
    ...
    """开始训练"""
    history = model.fit(x=X_profiling[:50000], y=Y_profiling[:50000, :9], batch_size=50, verbose=2,
                        epochs=epoch,
                        callbacks=callback
                        )
