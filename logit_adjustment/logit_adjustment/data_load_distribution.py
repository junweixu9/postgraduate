...
def read_data(leakage_model, data_arguementation, data_arguementation_level,
    ...
    if dataset == 'ASCAD':
        ...
        (X_profiling, X_attack), (Y_profiling, Y_attack), (plt_profiling, plt_attack), (
            ...
            "./ASCAD_data/ASCAD_databases/ASCAD.h5",
    ...
    if sigma_hw == 0 and sigma_id == 0:
        ...
        Y_profiling = np.concatenate(
            (to_categorical(Y_profiling, num_classes=classes),
             np.zeros((len(plt_profiling), 1)),
             SCA_util.Utility.compute_label_distribution(Y_profiling, leakage_model, 1, 0)
             ), axis=1)
