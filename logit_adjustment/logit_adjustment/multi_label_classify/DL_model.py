...
def ascad_r_hw_cnn(length, metric, loss):
    ...
    conv_branch1 = Conv1D(filters=4, kernel_size=1, activation='selu', padding='same', name='conv1x1')(input_layer)

    # 分支2: 7x7 卷积核
    conv_branch2 = Conv1D(filters=4, kernel_size=7, activation='selu', padding='same', name='conv7x7')(input_layer)

    # 分支3: 11x11 卷积核
    conv_branch3 = Conv1D(filters=4, kernel_size=11, activation='selu', padding='same', name='conv11x11')(input_layer)

    # --- 特征拼接 ---
    # 将三个并行分支的输出特征在通道维度上拼接起来
    concatenated_features = Concatenate(name='concatenate')([conv_branch1, conv_branch2, conv_branch3])

    x = Conv1D(8, 3, kernel_initializer='he_uniform', activation='selu', padding='same')(concatenated_features)
    x = AveragePooling1D(25, strides=25)(x)
    x = Flatten(name='flatten')(x)
