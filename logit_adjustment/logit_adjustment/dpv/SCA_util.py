...
def perform_attacks(offset, mask, all_valid_plt_attack, y_pred, output_metric, leakage_model, correct_key,
                    nb_traces_attacks,
                    shuffle=True):
    k_c = correct_key
    ...
    for i in range(num_of_attacks):
        if shuffle:
            l = list(zip(y_pred, all_valid_plt_attack, offset))
            random.shuffle(l)
            s1, s2, offset = list(zip(*l))
            ...
            offset = offset[:nb_traces_attacks]
            offset = np.array(offset)

        '''得到打乱之后的数据集，进行一次rank or corr的计算'''
        results = rank_compute(offset, mask, yibufen_attack_pred, yibufen_attack_plt, attack_byte, k_c, output_metric,
                               leakage_model,
                               nb_traces_attacks)
...
def rank_compute(offset, mask, yibufen_attack_pred, yibufen_attack_plt, attack_byte, k_c, output_metric, leakage_model,
                 nb_traces_attacks) \
        :
    ...
    for i in range(nb_traces):
        ...
        f = Utility.AES_Sbox()[np.bitwise_xor(range(256), int(yibufen_attack_plt[i, 0]))]
        g = np.bitwise_xor(f,
                           int(mask[int(offset[i, 0] + 1) % 16]))

        predicted_output = yibufen_attack_pred[i, Utility.calculate_HW(
            g)]
        #  并进行对应密钥得分的相乘
