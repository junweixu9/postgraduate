...
if __name__ == '__main__':
    ...
    sigma_hw = 0.5  # sigma for the HW leakage model
    ...
    companion_metric = None  # None/all/kl_loss_model
    ...
    wandb.init(
        ...
        project="SCA_logit_adjustment_project_search",
    ...
    if sigma_hw == 0 and sigma_id == 0:
        ...
        if attack_traces[-1, correct_key] > 0:
            ...
            print("GE:", attack_traces[-1, correct_key])
            for epoch in range(len(loss)):
                wandb.log({"train_loss": loss[epoch], "rank": all_rank_logs[epoch],
                           "corr": all_corr_logs[epoch]})
            wandb.log({"GE": attack_traces[-1, correct_key]})
        else:
            print("攻击成功")
            print("TGE0:", np.argmax(attack_traces[:, correct_key] < 1))
            for epoch in range(len(loss)):
                wandb.log({"train_loss": loss[epoch], "rank": all_rank_logs[epoch],
                           "corr": all_corr_logs[epoch]})
            wandb.log({"TGE0": np.argmax(attack_traces[:, correct_key] < 1)})
