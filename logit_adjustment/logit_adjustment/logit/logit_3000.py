...
def compute_adjustment(Y_profiling, tro):
    ...
    balance_factor = np.power(label_freq_array, tro - 1)
    ...
    return adjustments, balance_factor
...
def kl_adjustment_loss(y_true, y_pred):
    ...
    return K.sum(tf.multiply(y_true * K.log(y_true / y_pred), balance_factor), axis=-1)
...
if __name__ == '__main__':
    ...
    adjustments, balance_factor = compute_adjustment(Y_profiling, tro)
    adjustments_valid = tf.cast(adjustments, dtype=tf.double)

    log = open('F:/result/logit_ldl/a/3000.txt', mode='a',
               encoding='utf-8')
    print(file=log)
    print(file=log)
    """创建神经网络模型"""
