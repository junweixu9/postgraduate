...
def fl(y_true, y_pred):
    ...
    fl = K.mean(K.sum(loss, axis=-1))
    return fl


def flr_loss(y_true, y_pred):
    ce = fl(y_true, y_pred)
...
def build_model_mlp(hp):
    ...
    model.add(layers.Dense(units=hp.Choice('dense_' + str(0) + '_units'
    ...
    for j in range(hp.Int('n_dense_layers', 1, 7)):
        model.add(
            layers.Dense(units=hp.Choice('dense_' + str(j) + '_units', values=[10, 20, 50, 100, 200, 300, 400, 500]),
                         activation=activation,
                         ))
    model.add(Dense(9))
    loss_auto = flr_loss
    model.compile(optimizer=RMSprop(lr=hp.Choice('learning_rate', values=[1e-3, 5e-4, 1e-4])),
...
def build_model_cnn(hp):
    ...
    model.add(Flatten())
    for j in range(hp.Int('n_dense_layers', 1, 4)):
