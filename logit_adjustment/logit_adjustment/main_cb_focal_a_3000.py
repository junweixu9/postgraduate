...
import Util.DL_model_origin as DL_model
import numpy as np
...
def compute_class_balance_factor(Y_profiling, beta):
    """compute the base probabilities"""

    Y_profiling = np.sum(Y_profiling[:, :9], 0)
    betany = np.power(beta, Y_profiling)
    cb_factor = np.array((1 - beta) / (1 - betany), np.float64)
...
def HYBRID_loss(y_true, y_pred):
    ...
    # focal
    y_pred_cb_focal_factor = tf.nn.softmax(y_pred, 1)
    focal_factor = tf.pow(1 - y_pred_cb_focal_factor, gamma)
    cb_focal_factor = tf.multiply(focal_factor, cb_factor)

    # adjustment
    y_pred_adjustments = y_pred + 1 * adjustments
    y_pred_adjustments = tf.nn.softmax(y_pred_adjustments, 1)

    y_true = K.clip(y_true, K.epsilon(), 1)
    y_pred_cb_focal_factor = K.clip(y_pred_cb_focal_factor, K.epsilon(), 1)
    y_pred_adjustments = K.clip(y_pred_adjustments, K.epsilon(), 1)

    cb_focal_factor_loss = K.sum(tf.multiply(y_true * K.log(y_true / y_pred_cb_focal_factor), cb_focal_factor), axis=-1)
    adjustments_loss = K.sum(y_true * K.log(y_true / y_pred_adjustments), axis=-1)

    return 0.5 * cb_focal_factor_loss + 0.5 * adjustments_loss
    # return 1 * cb_focal_factor_loss
...
if __name__ == '__main__':
    ...
    tro = 1
    count = 0
    log = open('F:/result/cbfocal/a_newest/3000.txt', mode='a', encoding='utf-8')
    best_weights = None
    adjust_flag = False
    ...
    cb_factor = compute_class_balance_factor(Y_profiling, 0.999)
    cb_factor_valid = tf.cast(cb_factor, dtype=tf.double)
    ...
    model, batch_size, epoch_sota = DL_model.pick_SOAT(dataset, leakage_model, X_profiling.shape[1],
                                                       companion_metric,
                                                       HYBRID_loss, model=attack_model,
