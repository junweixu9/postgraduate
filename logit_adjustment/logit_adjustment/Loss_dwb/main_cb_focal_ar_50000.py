...
def compute_class_balance_factor(Y_profiling, beta):
    ...
    cb_factor = np.log(np.max(Y_profiling) / Y_profiling) + 1

    return cb_factor


def dwb_loss(y_true, y_pred):
    y_true = y_true[:, :classes]
    y_pred = tf.nn.softmax(y_pred, 1)

    cb_factor_power = tf.pow(cb_factor_valid, 1 - y_pred)
    cb_factor_power_y_true = tf.multiply(cb_factor_power, y_true)
    # p_1_p = tf.reduce_mean(tf.reduce_sum(tf.multiply(y_pred, 1 - y_pred), 1))

    loss = tk.backend.categorical_crossentropy(cb_factor_power_y_true, y_pred)
    return loss
...
if __name__ == '__main__':
    ...
    kl_loss_logs = []
    count = 0
    log = open('F:/result/dwb/ar/50000.txt', mode='a', encoding='utf-8')
    best_weights = None
    adjust_flag = False
    ...
    cb_factor_valid = tf.cast(cb_factor, dtype=tf.float32)
    ...
    model, batch_size, epoch_sota = DL_model.pick_SOAT(dataset, leakage_model, X_profiling.shape[1],
                                                       companion_metric,
                                                       dwb_loss, learning_rate, model=attack_model,
