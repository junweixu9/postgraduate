...
def feature_extractor_layer():
    ...
    x = Dense(872, activation='elu')(x)
    x = Dense(608, activation='elu')(x)
    feature = Dense(512, activation='elu')(x)
    # x = Dense(496, activation='relu')(img_input)
    # x = Dense(496, activation='relu')(x)
    # x = Dense(136, activation='relu')(x)
    # x = Dense(288, activation='relu')(x)
    ...
    optimizer = RMSprop(lr=0.0005)  # 0.0005
    model.compile(loss=adjustment_loss_centerloss, optimizer=optimizer)
...
def adjustment_loss_centerloss(y_true, y_pred):
    labels_batch = tf.math.argmax(y_true[:, :9], axis=1)
    x_batch = tf.reshape(y_pred, (tf.shape(y_pred)[0], -1))
    centers_batch = tf.gather(centers, labels_batch)
    ...
    x_correspond_centers_batch = tf.gather(centers, labels_batch)
    ...
    centers.assign_sub(center_update)
...
if __name__ == '__main__':
    ...
    centers = tf.Variable(tf.zeros([9, n_features]), name="centers",
                          trainable=False)
    ...
    model.fit(x={"x": X_profiling}, y=Y_profiling[:, :9],
