from data_load_ascad_3000 import read_data
...
def compute_class_balance_factor(Y_profiling, beta):
    ...
    label_freq_array = np.sum(Y_profiling[:, :9], 0)
    betany = np.power(beta, label_freq_array)
    cb_factor = np.array((1 - beta) / (1 - betany), np.float64)

    return cb_factor


def cb_focal_factor_loss(y_true, y_pred):
    ...
    return K.sum(tf.multiply(y_true * K.log(y_true / y_pred), cb_focal_factor), axis=-1)
...
if __name__ == '__main__':
    ...
    log = open('F:/result/cb_focal_ldl/c/5000.txt', mode='a',
               encoding='utf-8')
    print(file=log)
    print(file=log)
    """创建神经网络模型"""
    ...
    model, batch_size, epoch_sota = DL_model.pick_SOAT(dataset, leakage_model, X_profiling.shape[1],
                                                       companion_metric,
                                                       cb_focal_factor_loss, learning_rate, model=attack_model,
