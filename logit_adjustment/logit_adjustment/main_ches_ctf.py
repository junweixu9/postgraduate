from data_load_ches import read_data
    ...
    def on_epoch_end(self, epoch, logs=None):
        if self.validation:
            ...
            avg_rank_current, avg_attack_traces = perform_attacks(all_valid_plt_attack_metric, y_pred_valid_metric,
                                                                  'rank',
                                                                  leakage_model, dataset, num_traces_attacks)
            if avg_attack_traces[-1, 23] > 0:
                print("验证集攻击失败", file=log)
                print("验证集GE:", avg_attack_traces[-1, 23])
            else:
                print("验证集攻击成功", file=log)
                print("验证集TGE0:", np.argmax(avg_attack_traces[:, 23] < 1))
            logs['rank_val'] = avg_rank_current
            all_rank_valid_logs.append(avg_rank_current)
...
if __name__ == '__main__':
    ...
    kl_loss_logs = []
    count = 0
    log = open('C:/Users/Administrator/OneDrive/Desktop/result/experiment7.txt', mode='a', encoding='utf-8')
    best_weights = None
    tro = 1
    adjust_flag = False
    ...
    #     project="SCA_logit_adjustment_project_1",
    #
    #     # track hyperparameters and run metadata
    #     config={
    #         "learning_rate": learning_rate,
    #         "architecture": attack_model,
    #         "dataset": dataset,
    #         "epochs": epoch,
    #         "num_profiling_traces": num_profiling_traces,
    #         "num_attack_traces": num_traces_attacks,
    #         "sigma_hw": sigma_hw,
    #         "sigma_id": sigma_id,
    #         "adjust_flag": adjust_flag
    #     }
    # )
    # binomial_distribution = np.zeros(9)
    # for i in range(9):
    #     binomial_distribution[i] = comb(8, i)
    # binomial_distribution = tf.cast(binomial_distribution, dtype=tf.double)
    ...
    if sigma_hw == 0 and sigma_id == 0:
        ...
        callback = [rank(validation=(X_profiling[:5000], plt_profiling[:5000], Y_profiling[:5000])),
                    # corr_max(validation=(X_profiling[:5000], plt_profiling[:5000], Y_profiling[:5000])),
        ...
        # kl_loss = history.history['kl_loss_model']
        # for epoch in range(len(loss)):
        #     wandb.log({"train_loss": loss[epoch], "valid_loss": loss_logs[epoch], "kl_train_loss": kl_loss[epoch],
        #                "kl_valid_loss": kl_loss_logs[epoch], "rank": all_rank_logs[epoch],
        #                "corr": all_corr_logs[epoch]})
        ...
        if attack_traces[-1, correct_key] > 0:
            ...
            print("learning_rate_", learning_rate,
                  ...
                  "___sigma_id_", sigma_id, file=log)
            print("攻击失败", file=log)
            print("GE:", attack_traces[-1, correct_key], file=log)
            for epoch in range(len(loss)):
                print("轮数", epoch, "__train_loss_", loss[epoch], "__rank_", all_rank_valid_logs[epoch], file=log)
            log.close()
        ...
        filename = model_root + test_info
        """开始训练"""
        history = model.fit(x=X_profiling[5000:], y=Y_profiling[5000:, :9], batch_size=batch_size, verbose=2, epochs=epoch,
                            callbacks=callback)
