...
def load_chesctf(database_file, leakage_model='HW', key_info=False, profiling_traces=50000, attack_trace=10000):
    ...
    Y_profiling[:profiling_traces], Y_attack[:attack_trace]), \
...
def load_aeshd(AESHD_data_folder, profiling_traces=50000, leakage_model='HW', target_byte=11):
    def xor_bytes(a, b):
    ...
    in_file = h5py.File(r'C:\workspace\code\logit_adjustment\logit_adjustment\ASCAD_data\ASCAD_databases\aes_hd.h5', "r")
    ...
    if leakage_model == 'HW':
        Y_profiling = calculate_HW(Y_profiling)
        Y_attack = calculate_HW(Y_attack)
    return (X_profiling[:profiling_traces], X_attack), (Y_profiling[:profiling_traces], Y_attack), (
