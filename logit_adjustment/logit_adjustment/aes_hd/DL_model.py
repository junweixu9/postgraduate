...
def pick_SOAT(data, leakage_model, length, metric, loss, learning_rate, model='MLP', model_size=64):
    if model == 'CNN':
        ...
        elif data == 'AES_HD' and leakage_model == 'HW':
            batch_size = 128
            epoch = 50
            return cnn_architecture_aes_hd(length, metric, loss), batch_size, epoch
...
def cnn_architecture_aes_hd(length, metric, loss):
...
def noConv1_ascad_desync_0(length, metric, loss):
    trace_input = Input(shape=(length, 1))
