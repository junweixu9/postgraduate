...
def perform_attacks(all_valid_plt_attack, y_pred, output_metric, leakage_model, correct_key, nb_traces_attacks,
                    shuffle=True):
...
def rank_compute(yibufen_attack_pred, yibufen_attack_plt, attack_byte, k_c, output_metric, leakage_model,
    ...
    for i in range(nb_traces):
        ...
        hw_g = Utility.calculate_HW(
            g)


        predicted_output = yibufen_attack_pred[i, hw_g]

        predicted_output_adjust = compute_values(predicted_output, result, hw_g)
        #  并进行对应密钥得分的相乘
        key_log_prob_accu += predicted_output_adjust
