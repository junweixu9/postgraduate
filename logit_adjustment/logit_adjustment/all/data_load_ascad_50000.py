...
import SCA_dataset as SCA_dataset
import sys
from sklearn.preprocessing import StandardScaler
import numpy as np
from tensorflow.keras.utils import to_categorical
import SCA_util as SCA_util
...
def read_data(leakage_model, data_arguementation, data_arguementation_level,
    ...
    if dataset == 'ASCAD':
        ...
        num_profiling_traces = 6000
        (X_profiling, X_attack), (Y_profiling, Y_attack), (plt_profiling, plt_attack), (
            key_profiling, key_attack) = SCA_dataset.load_ascad(
            "C:/workspace/code/logit_adjustment/logit_adjustment/ASCAD_data/ASCAD_databases/ASCAD.h5",
            leakage_model=leakage_model,
            profiling_traces=num_profiling_traces,
            key_info=True)
    elif dataset == 'ASCAD_rand':
        data_root = ('C:/workspace/code/logit_adjustment/logit_adjustment/ASCAD_data/ASCAD_databases/ascad'
                     '-variable.h5')
        correct_key = 34
        attack_byte = 2
        num_profiling_traces = 6000
        ...
        data_root = 'C:/workspace/code/logit_adjustment/logit_adjustment/ASCAD_data/ASCAD_databases/ches_ctf.h5'
        correct_key = 46
        attack_byte = 0
        num_profiling_traces = 6000
        num_attack_traces = 10000
        ...
        (X_profiling, X_attack), (Y_profiling_ID, Y_attack_ID), (plt_profiling, plt_attack) = SCA_dataset.load_aeshd(
            data_root, leakage_model=leakage_model,
    ...
    x = Y_profiling[:5000].astype(float)
    n = len(x)

    # Eq.(9)
    xbar = x.mean()
    sigma_hat = np.sqrt(np.mean((x - xbar) ** 2) + 1e-12)

    # Eq.(8)  (or 1.06 * sigma_hat * n**(-1/5))
    h = ((4 * sigma_hat ** 5) / (3 * n)) ** (1 / 5)
