...
class all(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if self.validation:
            ...
            # print()
def ascad_r_hw_cnn_rs(extra_loss_param, length, metric, loss, learning_rate):
    intermediate_output = []
...
def ches_f_hw_cnn_rs(extra_loss_param, length, metric, loss, learning_rate):
    img_input = Input(shape=(length, 1))
    intermediate_output = []
...
loss_type = "soft_nn"  # “center_loss”  "soft_nn"
...
(X_profiling, X_attack), (Y_profiling, Y_attack), (
    ...
    sigma_hw, sigma_id)
distillation_model = ches_f_hw_cnn_rs(extra_loss_param,X_profiling.shape[1],None,flr_adjustment_loss,1e-5)
...
history = distillation_model.fit(
    x=X_profiling[:50000], y=Y_profiling[:50000, :9],
    batch_size=128,
    epochs=50,
)
