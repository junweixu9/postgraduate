...
def rank_compute(yibufen_attack_pred, yibufen_attack_plt, attack_byte, k_c, output_metric, leakage_model,
    ...
    yibufen_attack_pred_int = np.argmax(yibufen_attack_pred, axis=1)
    acc_condition = np.zeros((nb_traces, 256))
    for i in range(nb_traces):
        ...
        acc_condition[i] = np.equal(yibufen_attack_pred_int[i], g)
        #  并进行对应密钥得分的相乘
    ...
    # acc = np.mean(acc_condition, axis=0)
    # formatted_acc = np.round(acc, 3)
    # print(formatted_acc)
    if output_metric == 'corr':
