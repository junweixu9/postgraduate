...
if __name__ == '__main__':
    ...
    score_prime = np.zeros((num_traces_attacks, 256))
    predictions_prime = np.zeros((5000, 9))

    #
    number_of_models = 9
    number_of_best_models = 5
    ge_all_validation = []
    k_ps_all = []
    #
    for i in range(1, number_of_models + 1):
        model = ascad_r_hw_cnn(X_profiling.shape[1], None, adjustment_loss, 5e-3)
        """最佳模型的存储地址"""
        ...
        model_root = './' + test_info + str(i) + '.h5'
        model.load_weights(model_root)
        predictions = model.predict(X_attack[5000:])
        predictions = tf.nn.softmax(predictions)
        predictions_prime = predictions_prime + predictions
        attack_traces, kp_krs = perform_attacks(plt_attack[5000:], predictions, "attack_traceswithscore",
                                                leakage_model, dataset, num_traces_attacks)

        print(str(i) + '/' + str(number_of_models))
    ...
    # attack_tracesin = perform_attacks(plt_attack[:5000], predictions_prime / number_of_models, "attack_traces",
    #                                   leakage_model, dataset, num_traces_attacks)
    # if attack_tracesin[-1, correct_key] > 0:
    #     print("攻击失败")
    #     print("GE:", attack_tracesin[-1, correct_key])
    #
    # else:
    #     print("攻击成功")
    #     print("TGE0:", np.argmax(attack_tracesin[:, correct_key] < 1))
