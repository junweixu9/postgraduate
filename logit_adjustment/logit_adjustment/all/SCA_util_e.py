import numpy as np
import scipy.stats as ss
from tensorflow.keras import backend as K
from scipy import stats
import sys
import random
import tensorflow as tf


class Utility:
    def AES_Sbox():
        return np.array([
            
        ])

    def hw():
        return np.array([bin(x).count("1") for x in range(256)])

    def labelize(plaintexts, keys):
        return Utility.AES_Sbox()[plaintexts ^ keys]

    def calculate_HW(data):
        if isinstance(data, int):
            print('Input must be an array')
            sys.exit(-1)
        if data.ndim == 1:
            return Utility.hw()[data]
        else:
            return np.reshape([Utility.hw()[data.ravel()]], np.shape(data))

    ''' rk_key_all_traces函数的作用是对输入的rank_array中的每一行进行密钥排名。具体来说，它对每一行中的元素按降序排名，然后将排名结果存储在一个新的数组中。'''

    def rk_key_all_traces(rank_array):
        container = np.empty(rank_array.shape, dtype=int)
        for k, row in enumerate(rank_array):
            container[k] = ss.rankdata(-row, method='dense') - 1
        return container

    def compute_label_distribution(labels, leakage_model, sigma_hw, sigma_id):
        if leakage_model == 'HW':
            sigma = sigma_hw
            classes = 9
        else:
            sigma = sigma_id
            classes = 256
        container = np.zeros((len(labels), classes), dtype=np.float32)
        # Label Distribution Learning
        for idx, label in enumerate(labels):
            container[idx] = [1 / (sigma * np.sqrt(2 * np.pi)) * np.exp(- (bins - label) ** 2 / (2 * sigma ** 2)) for
                              bins in range(classes)]
        return container


def perform_attacks(all_valid_plt_attack, all_son_model_y_pred, output_metric, leakage_model, dataset,
                    nb_traces_attacks, shuffled_indices_list,sorted_indices=None, number_of_best_models=0, shuffle=True):
    if dataset == "ASCAD":
        k_c = 224
        attack_byte = 2
    elif dataset == "ASCAD_rand":
        k_c = 34
        attack_byte = 2
    elif dataset == "CHES_CTF":
        k_c = 46
        attack_byte = 0
    else:
        k_c = 200
        attack_byte = 11

    num_of_attacks = 50

    if output_metric == "rank":
        all_rank_evol = np.zeros((num_of_attacks, 1))
        all_key_attack_traces = np.zeros((num_of_attacks, nb_traces_attacks, 256))
    elif output_metric == "corr":
        all_corr_evol = np.zeros((num_of_attacks, 1))
    elif output_metric == "all_attack_traces":
        all_key_attack_traces = np.zeros((num_of_attacks, nb_traces_attacks, 256))
        all_key_attack_traces_best = np.zeros((num_of_attacks, nb_traces_attacks, 256))
    elif output_metric == "one_attack_traces":
        #  将进行 nb_attacks 次计算得到的矩阵（该矩阵的第i行是使用i条攻击能量轨迹数得到的猜测向量），计算平均值，消除随机性
        all_key_attack_traces = np.zeros((num_of_attacks, nb_traces_attacks, 256))
    elif "attack_traces" in output_metric:
        all_key_attack_traces = np.zeros((num_of_attacks, nb_traces_attacks, 256))
    elif output_metric == "all":
        all_rank_evol = np.zeros((num_of_attacks, 1))
        all_key_attack_traces = np.zeros((num_of_attacks, nb_traces_attacks, 256))
        all_corr_evol = np.zeros((num_of_attacks, 1))
    #  打乱攻击数据集
    i = 0
    for shuffled_indices in shuffled_indices_list:
        if output_metric == "all_attack_traces":

            shuffled_indices_np = np.array(shuffled_indices)

            yibufen_attack_plt = all_valid_plt_attack[shuffled_indices_np]
            yibufen_attack_plt = np.array(yibufen_attack_plt)

            all_son_model_pred_selected = [
                [each_son_modelpred[i] for i in shuffled_indices_np]  # Extract specific indices for each row
                for each_son_modelpred in all_son_model_y_pred
            ]
            all_son_model_pred_selected = np.array(all_son_model_pred_selected)

            results, results_best = rank_compute(all_son_model_pred_selected, yibufen_attack_plt, attack_byte, k_c,
                                                 output_metric,
                                                 leakage_model, nb_traces_attacks,
                                                 sorted_indices[:number_of_best_models])
        elif output_metric == "one_attack_traces":

            shuffled_indices_np = np.array(shuffled_indices)

            yibufen_attack_plt = all_valid_plt_attack[shuffled_indices_np]
            yibufen_attack_plt = np.array(yibufen_attack_plt)

            shuffled_indices_tf = tf.convert_to_tensor(shuffled_indices, dtype=tf.int32)
            all_son_model_pred_selected = tf.gather(all_son_model_y_pred, shuffled_indices_tf, axis=0)
            all_son_model_pred_selected = np.array(all_son_model_pred_selected)
            results = rank_compute(all_son_model_pred_selected, yibufen_attack_plt, attack_byte, k_c, output_metric,
                                   leakage_model,
                                   nb_traces_attacks)

        #  把进行一次计算的rank or corr存储到新的向量中，以便做num_of_attacks的平均值
        if output_metric == 'corr':
            all_corr_evol[i] = results
        elif output_metric == 'rank':
            #  计算一次指标需进行 nb_attacks 次数据的计算
            all_rank_evol[i] = results[nb_traces_attacks - 1, k_c]
            all_key_attack_traces[i] = results
        elif output_metric == "all_attack_traces":
            all_key_attack_traces[i] = results
            all_key_attack_traces_best[i] = results_best
        elif output_metric == "one_attack_traces":
            #  将进行 nb_attacks 次计算得到的矩阵（该矩阵的第i行是使用i条攻击能量轨迹数得到的猜测向量），计算平均值，消除随机性
            all_key_attack_traces[i] = results
        elif output_metric == "all":
            all_rank_evol[i] = results[0][nb_traces_attacks - 1, k_c]
            all_key_attack_traces[i] = results[0]
            all_corr_evol[i] = results[1]
        i = i + 1

    """进行num_of_attacks次打乱的攻击数据集后，进行平均值计算"""
    if output_metric == 'corr':
        return np.mean(all_corr_evol, axis=0)
    elif output_metric == 'rank':
        #  将进行 nb_attacks 次计算得到的矩阵（该矩阵的第i行是使用i条攻击能量轨迹数得到的猜测向量），计算平均值，消除随机性
        return (np.mean(all_rank_evol, axis=0)), np.mean(all_key_attack_traces, axis=0)
    elif output_metric == "all":
        return (np.mean(all_rank_evol, axis=0)), np.mean(all_key_attack_traces, axis=0), np.mean(all_corr_evol, axis=0)
    elif output_metric == "all_attack_traces":
        return np.mean(all_key_attack_traces, axis=0), np.mean(all_key_attack_traces_best, axis=0)
    elif output_metric == "one_attack_traces":
        #  将进行 nb_attacks 次计算得到的矩阵（该矩阵的第i行是使用i条攻击能量轨迹数得到的猜测向量），计算平均值，消除随机性
        return np.mean(all_key_attack_traces, axis=0)


def KD_distribution(yibufen_plt_attack, attack_byte, k_c):
    num_plt_attack = len(yibufen_plt_attack)
    k = np.zeros((num_plt_attack, 256))
    variance = np.zeros((256, 1))
    for i in range(num_plt_attack):
        # (num_plt_attack,256)
        k[i] = Utility.calculate_HW(
            Utility.AES_Sbox()[np.bitwise_xor(range(256), int(yibufen_plt_attack[i, attack_byte]))])
    k = k.T  # (256,num_plt_attack)
    for i in range(256):
        variance[i] = np.sum(abs(np.power(k[k_c] - k[i], 2)))
    KD_rank = ss.rankdata(variance)
    return KD_rank


def compute_values(predicted_output, result, g):
    return predicted_output / result[g]


def rank_compute(all_son_model_pred_selected, yibufen_attack_plt, attack_byte, k_c, output_metric, leakage_model,
                 nb_traces_attacks, best_sorted_indices=None):
    if output_metric == "all_attack_traces":
        (nb_traces, nb_hyp) = all_son_model_pred_selected[0].shape
        num_son_model = len(all_son_model_pred_selected)

    elif output_metric == "one_attack_traces":
        (nb_traces, nb_hyp) = all_son_model_pred_selected.shape
        num_son_model = 1

    if output_metric == "corr" or "all":
        # 得到密钥分布的排名
        KD_rank = KD_distribution(yibufen_attack_plt, attack_byte, k_c)

    all_son_model_key_log_prob_evol = np.zeros((num_son_model, nb_traces, 256))
    all_son_model_sum_key_log_prob_evol = np.zeros((nb_traces, 256))

    #  将得到的概率分布转换为log分数
    yibufen_attack_pred = np.log(
        np.where(all_son_model_pred_selected <= K.epsilon(), K.epsilon(), all_son_model_pred_selected))
    index_son_model = 0
    if output_metric == "all_attack_traces":
        for son_model in yibufen_attack_pred:
            key_log_prob_accu = np.zeros(256)
            for i in range(nb_traces):
                #   从log的概率分布取出所有密钥根据汉明重量计算出相应标签的log得分
                g = Utility.calculate_HW(
                    Utility.AES_Sbox()[np.bitwise_xor(range(256), int(yibufen_attack_plt[i, attack_byte]))])
                predicted_output = son_model[i, g]
                #  并进行对应密钥得分的相乘
                key_log_prob_accu += predicted_output
                all_son_model_key_log_prob_evol[index_son_model, i] = key_log_prob_accu

            index_son_model = index_son_model + 1
        all_son_model_sum_key_log_prob_evol = np.sum(all_son_model_key_log_prob_evol, axis=0)
        all_son_model_sum_key_log_prob_evol_best = np.sum(all_son_model_key_log_prob_evol[best_sorted_indices], axis=0)

    elif output_metric == "one_attack_traces":
        key_log_prob_accu = np.zeros(256)
        for i in range(nb_traces):
            #   从log的概率分布取出所有密钥根据汉明重量计算出相应标签的log得分
            g = Utility.calculate_HW(
                Utility.AES_Sbox()[np.bitwise_xor(range(256), int(yibufen_attack_plt[i, attack_byte]))])
            predicted_output = yibufen_attack_pred[i, g]
            #  并进行对应密钥得分的相乘
            key_log_prob_accu += predicted_output
            #  用i条攻击轨迹得到的未经过排名的猜测向量 guessing entropy ==  key_log_prob_evol[i]
            all_son_model_sum_key_log_prob_evol[i] = key_log_prob_accu  # (nb_traces,256)

    if output_metric == 'corr':
        '''rk_key_all_traces 函数的作用是对输入的未经过排名的猜测向量 key_log_prob_evol 中的每一行进行密钥排名。具体来说，它对每一行中的元素按降序排名，然后将排名结果存储在一个新的数组中。'''
        rank = Utility.rk_key_all_traces(all_son_model_sum_key_log_prob_evol)
        corr, _ = stats.spearmanr(rank[nb_traces_attacks - 1], KD_rank)
        return corr
    elif output_metric == 'rank':
        rank = Utility.rk_key_all_traces(all_son_model_sum_key_log_prob_evol)
        return rank
    elif output_metric == 'one_attack_traces':
        attack = Utility.rk_key_all_traces(all_son_model_sum_key_log_prob_evol)
        return attack
    elif output_metric == 'all_attack_traces':
        attack = Utility.rk_key_all_traces(all_son_model_sum_key_log_prob_evol)
        attack_best = Utility.rk_key_all_traces(all_son_model_sum_key_log_prob_evol_best)
        return attack, attack_best
    elif output_metric == "all":
        rank = Utility.rk_key_all_traces(all_son_model_sum_key_log_prob_evol)
        corr, _ = stats.spearmanr(rank[nb_traces_attacks - 1], KD_rank)
        return rank, corr
