...
class all(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if self.validation:
            ...
            # y_pred_valid_metric = y_pred_valid_metric - adjustments
...
if __name__ == '__main__':
    ...
    log = open('F:/result/cnn/c/0logit_curve_700.txt', mode='a',
               encoding='utf-8')
    ...
    def generate_image(text="程序已跑完", output_path="completed_matplotlib.png"):
        # 设置图像参数
        fig = plt.figure(figsize=(6, 3), dpi=200)  # 图像尺寸（宽600px，高300px）
