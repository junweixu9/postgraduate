...
if __name__ == '__main__':
    ...
    lr_manager = OneCycleLR(len(X_attack[:5000]), 400, learning_rate, end_percentage=0.2, scale_percentage=0.1,
    ...
    """开始训练"""
    history = model.fit(x=X_profiling[:50000], y=Y_profiling[:50000, :9], batch_size=400, verbose=2,
                        epochs=epoch,
                        callbacks=callback
                        )
    ...
    model_root = 'E:/logit_backup/ensemble_model/' + test_info + experiment_time_mid + '.h5'
    ...
    # log.close()
    #
    def generate_image(text="程序已跑完，记得更改模型名字", output_path="completed_matplotlib.png"):
        # 设置图像参数
