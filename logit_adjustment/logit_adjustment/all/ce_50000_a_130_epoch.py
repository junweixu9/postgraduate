...
def resnet_module(x, num_filters):  # 将num_filters作为参数
    ...
    res = Conv1D(
        filters=num_filters, kernel_size=25, strides=2, padding="same", name="res_conv_1"
    )(res)

    res = BatchNormalization(name="res_bn_2")(res)
    res = Activation("selu", name="res_selu_2")(res)
    res = Conv1D(
        filters=num_filters, kernel_size=25, strides=1, padding="same", name="res_conv_2"
    )(res)
...
def build_optimized_clrm_model(input_shape=(700, 1), num_classes=9):
    ...
    # --- 优化的 CNN 模块 (滤波器减半) ---
    x = Conv1D(filters=64, kernel_size=25, strides=2, padding="same", name="conv1d_1")(input_layer)  # 128 -> 64
    x = BatchNormalization(name="bn_1")(x)
    x = Activation("selu", name="selu_1")(x)
    x = MaxPooling1D(pool_size=2, strides=2, name="maxpool_1")(x)

    x = Conv1D(filters=128, kernel_size=25, strides=2, padding="same", name="conv1d_2")(x)  # 256 -> 128
    x = BatchNormalization(name="bn_2")(x)
    x = Activation("selu", name="selu_2")(x)
    x = MaxPooling1D(pool_size=2, strides=2, name="maxpool_2")(x)

    x = Conv1D(filters=256, kernel_size=25, strides=2, padding="same", name="conv1d_3")(x)  # 512 -> 256
    ...
    # dense3 = Dense(units=16, activation="relu", name="dense_3")(dense2)
    ...
    # --- 【必须】修改输出层为9类 ---
    output_layer = Dense(units=num_classes, name="output_layer")(dense2)
...
class all(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if self.validation:
            ...
            Y_attack_valid = Y_attack_valid[:5000, :9]
...
if __name__ == '__main__':
    ...
    companion_metric = "acc"  # None/all/kl_loss_model/'categorical_accuracy'
    ...
    #
    # _, batch_size, epoch_sota = DL_model.pick_SOAT(dataset, leakage_model, X_profiling.shape[1],
    #                                                companion_metric,
    ...
    lr_manager = OneCycleLR(len(X_attack[:5000]), 128, learning_rate, end_percentage=0.05, scale_percentage=0.11,
