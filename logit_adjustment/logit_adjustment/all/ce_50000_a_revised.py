...
class all(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if self.validation:
            ...
            log = open(r"C:\paper_final_revise\each_bit.txt", mode='a',
                       encoding='utf-8')
...
if __name__ == '__main__':
    ...
    attack_traces = perform_attacks(plt_attack[5000:], predictions, "attack_traces",
                                    leakage_model, dataset, num_traces_attacks, log, True)
    ...
    y_pred_valid_metric_int = tf.argmax(predictions[:2000, :9], axis=-1)
