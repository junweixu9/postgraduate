...
def adjustment_cer_loss(y_true, y_pred):
    y_true = y_true[:, :classes]

    if adjust_flag:
        y_pred_adustment = y_pred + 1 * adjustments

    y_pred_adustment_softmax = tf.nn.softmax(y_pred_adustment, 1)
    loss = tk.backend.categorical_crossentropy(y_true, y_pred_adustment_softmax)

    num_of_attacks = 10
    fake_k_store = 0
    for i in range(num_of_attacks):
        shuffled_y_true = tf.random.shuffle(y_true)
        fake_k_loss = tf.keras.losses.categorical_crossentropy(shuffled_y_true, y_pred_adustment_softmax)
        fake_k_store = fake_k_store + fake_k_loss
    average_fake_k_loss = fake_k_store / num_of_attacks
    loss_cer = loss / (average_fake_k_loss + 1e-40)
...
if __name__ == '__main__':
    ...
    model, batch_size, epoch_sota = DL_model.pick_SOAT(dataset, leakage_model, X_profiling.shape[1],
                                                       companion_metric,
                                                       adjustment_cer_loss, learning_rate, model=attack_model,
    ...
    # model_root = 'E:/logit_backup/ensemble_model/' + "datasetASCAD_rand_leakage_modelHW_epoch50_batch_size128_outputalltime_is8_14_28cer.h5"
    ...
    attack_traces = perform_attacks(plt_attack[5000:], predictions, "attack_traces",
                                    leakage_model, dataset, num_traces_attacks)
    log = open('F:/result/cnn/a/logit_curve_3000_new.txt', mode='a',
               encoding='utf-8')
    ...
    loss = history.history['loss']
    # model_root = 'E:/logit_backup/ensemble_model/' + test_info + experiment_time_mid + 'cer.h5'
