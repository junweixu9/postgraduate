...
Original file is located at
    https://colab.research.google.com/drive/1t8r90jhkdDn-oUfXhryWh1uI9-4do3L8
"""
...
class SCAMetrics:       #s box
    ...
    def aes_labelize_ge_sr(self, trace_data, byte, key, leakage):
        pt_ct = [row[byte] for row in trace_data]  #pt is stored in cell #0-> #15
                                                   #ct is stored in cell #16->31
        key_byte = np.full(len(pt_ct), key)  # a new array of len(pt_ct) filled with value key[byte] #keys are stored from cell #32-> #47
    ...
    def ge_and_sr(self, runs, model, param, leakage_model, byte, x_test, test_trace_data, step, fraction,correct_key):
        ...
        for key_byte_hypothesis in range(0, 256): #loop is running 256 times => number of classes
            ...
            labels_key_hypothesis[key_byte_hypothesis][:] = self.aes_labelize_ge_sr(test_trace_data, byte, key_byte_hypothesis, leakage_model)
            #labeling is done for all nt at once
        # ---------------------------------------------------------------------------------------------------------#
        ...
        for run in range(runs):
            ...
            for index in range(nt_kr): #using the fraction of traces from nt                #1e-36 avoid taking log 0
                ...
                if (index + 1) % step == 0:  #loop is entered after every step (of x-axis if it were a plot)
                    key_ranking_good_key = list(key_probabilities_sorted).index(correct_key) + 1 #We take out index of the good key from sorted prob & we add 1 to it
            ...
            print(                                                                      #GE is calculated after each run cumulatively
                "KR: {} | GE for correct key ({}): {})".format(run, correct_key, key_ranking_sum[nt_interval - 1] / (run + 1)))
