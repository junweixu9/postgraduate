...
from sca_metrics_aes_rd import SCAMetrics
...
class EnsembleAES:
    ...
    def run_cnn(self, X_profiling, Y_profiling, X_validation, Y_validation, X_attack, Y_attack, plt_validation,
        ...
        model = NeuralNetwork().cnn_random(self.classes, X_attack.shape[1], activation, neurons, conv_layers,
                                           filters,
    ...
    def run_ensemble(self, number_of_models, number_of_best_models):
        ...
        real_key = np.load("C:/workspace/code/logit_adjustment/logit_adjustment/aes_rd/key.npy")
        correct_key = real_key[0]

        # Load the profiling traces
        X_profiling = np.load(
            "C:/workspace/code/logit_adjustment/logit_adjustment/aes_rd/profiling_traces_AES_RD.npy")
        Y_profiling = np.load(
            "C:/workspace/code/logit_adjustment/logit_adjustment/aes_rd/profiling_labels_AES_RD.npy")
        X_attack = np.load(
            "C:/workspace/code/logit_adjustment/logit_adjustment/aes_rd/attack_traces_AES_RD.npy")
        Y_attack = np.load(
            "C:/workspace/code/logit_adjustment/logit_adjustment/aes_rd/attack_labels_AES_RD.npy")
        plt_profiling = np.load(
            "C:/workspace/code/logit_adjustment/logit_adjustment/aes_rd/profiling_plaintext_AES_RD.npy")
        plt_attack = np.load(
            "C:/workspace/code/logit_adjustment/logit_adjustment/aes_rd/attack_plaintext_AES_RD.npy")

        X_validation = X_attack[:5000]
        Y_validation = Y_attack[:5000]
        plt_validation = plt_attack[:5000]

        X_attack = X_attack[5000:]
        Y_attack = Y_attack[5000:]
        plt_attack = plt_attack[5000:]
        ...
        Y_validation = calculate_HW(Y_validation)
