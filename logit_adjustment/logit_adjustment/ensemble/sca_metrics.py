...
class SCAMetrics:       #s box
    ...
    def aes_labelize_ge_sr(self, trace_data, byte, key, leakage):
        ...
        key_byte = np.full(len(pt_ct), key[byte])  # a new array of len(pt_ct) filled with value key[byte] #keys are stored from cell #32-> #47
    ...
    def ge_and_sr(self, runs, model, param, leakage_model, byte, x_test, test_trace_data, step, fraction):
        ...
        for key_byte_hypothesis in range(0, 256): #loop is running 256 times => number of classes
            ...
            labels_key_hypothesis[key_byte_hypothesis][:] = self.aes_labelize_ge_sr(test_trace_data, byte, key_h, leakage_model)
