...
def resnet_module(x, num_filters=256):
    """
    根据论文中的图8定义带有自适应软阈值的残差网络模块。
    """
    # 主路径
    ...
    threshold_dense1 = Dense(units=128, activation="relu", name="res_dense_1_for_threshold")(avg_pool)
...
def build_clrm_model(input_shape=(700, 1), num_classes=256):
    """
    根据论文中描述的架构构建CLRM模型。
    """
    ...
    # --- CNN 模块 ---
    x = Conv1D(filters=128, kernel_size=3, strides=2, padding="same", name="conv1d_1")(input_layer)
    x = BatchNormalization(name="bn_1")(x)
    x = Activation("selu", name="selu_1")(x)
    x = MaxPooling1D(pool_size=2, strides=2, name="maxpool_1")(x)

    x = Conv1D(filters=256, kernel_size=3, strides=2, padding="same", name="conv1d_2")(x)
    x = BatchNormalization(name="bn_2")(x)
    x = Activation("selu", name="selu_2")(x)
    x = MaxPooling1D(pool_size=2, strides=2, name="maxpool_2")(x)

    x = Conv1D(filters=512, kernel_size=3, strides=2, padding="same", name="conv1d_3")(x)
    x = BatchNormalization(name="bn_3")(x)
    x = Activation("selu", name="selu_3")(x)
    x = MaxPooling1D(pool_size=2, strides=2, name="maxpool_3")(x)

    # --- LSTM 模块 ---
    lstm_out = Bidirectional(LSTM(128, return_sequences=True, name="bidirectional_lstm"))(x)
    ...
    # --- 残差网络模块 ---
    resnet_out = resnet_module(pool_after_lstm, num_filters=256)
    ...
    # --- 修正点 ---
    # 将变量名 `dropout` 修改为 `dropout_output`
    dropout_output = Dropout(0.4, name="dropout_layer")(dense2)
    output_layer = Dense(units=num_classes, name="output_layer")(dropout_output)

    # 创建模型
    model = Model(inputs=input_layer, outputs=output_layer, name="CLRM_Model")
...
if __name__ == '__main__':
    ...
    model = build_clrm_model(input_shape=(700, 1), num_classes=256)
    model.compile(loss=adjustment_loss, optimizer='adam', metrics=None)
    model.summary()

    lr_manager = OneCycleLR(len(X_attack[:5000]), batch_size, learning_rate, end_percentage=0.05, scale_percentage=0.11,
    ...
    """开始训练"""
    history = model.fit(x=X_profiling[:50000], y=Y_profiling[:50000, :256], batch_size=batch_size, verbose=2,
                        epochs=epoch,
                        callbacks=callback
                        )
