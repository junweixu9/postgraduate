...
def create_cbapd_model(input_shape=(700, 1), dropout_rate=0.0):
    """
    Constructs the CBAPD network model as described in the paper.

    Args:
        input_shape (tuple): The shape of the input power traces.
                             The paper uses (700, 1) for ASCAD datasets.
        dropout_rate (float): The dropout rate to be used in the dropout layers.
                              The paper specifies this is used for desynchronized datasets.

    Returns:
        tensorflow.keras.models.Model: The compiled CBAPD model.
    """
    trace_input = Input(shape=input_shape, name='trace_input')

    # The paper mentions 5 blocks of Conv, Batch Norm, Activation, and Pooling
    # Block 1
    # The first convolutional layer uses 64 filters.
    x = Conv1D(filters=64, kernel_size=11, strides=1, padding='same', activation=None)(trace_input)
    # The activation function is removed from the convolutional layer and placed after batch normalization[cite: 117].
    x = BatchNormalization()(x)
    # The paper indicates the Selu activation function performs best[cite: 176].
    x = Activation('selu')(x)
    # Average pooling is used with a pool size and stride of 2[cite: 197].
    x = AveragePooling1D(pool_size=2, strides=2)(x)

    # Block 2
    # Subsequent convolutional layers double the number of filters.
    x = Conv1D(filters=128, kernel_size=11, strides=1, padding='same', activation=None)(x)
    x = BatchNormalization()(x)
    x = Activation('selu')(x)
    x = AveragePooling1D(pool_size=2, strides=2)(x)

    # Block 3
    x = Conv1D(filters=256, kernel_size=11, strides=1, padding='same', activation=None)(x)
    x = BatchNormalization()(x)
    x = Activation('selu')(x)
    x = AveragePooling1D(pool_size=2, strides=2)(x)

    # Block 4
    x = Conv1D(filters=512, kernel_size=11, strides=1, padding='same', activation=None)(x)
    x = BatchNormalization()(x)
    x = Activation('selu')(x)
    x = AveragePooling1D(pool_size=2, strides=2)(x)

    # Block 5
    # The last convolutional layer has the same number of filters as the previous one.
    x = Conv1D(filters=512, kernel_size=11, strides=1, padding='same', activation=None)(x)
    x = BatchNormalization()(x)
    x = Activation('selu')(x)
    x = AveragePooling1D(pool_size=2, strides=2)(x)
    ...
    # Two fully connected layers with two dropout layers.
    # The paper specifies 4096 neurons for the fully connected layers, using the ReLU activation function,
    # inspired by AlexNet.
    x = Dense(4096, activation='relu')(x)
    # Dropout layers are added for datasets with countermeasures like desynchronization.
    # The paper suggests a rate of 0.2 for ASCAD_desync50 and 0.3 for ASCAD_desync100.
    if dropout_rate > 0.0:
    ...
    # The output layer has 256 neurons for the 256 possible key values,
...
# Example of how to create the model for the ASCAD_desync50 dataset
# The input shape is 700 sample points per trace[cite: 69].
# The dropout rate for this dataset is 0.2.
# cbapd_desync50_model = create_cbapd_model(input_shape=(700, 1), dropout_rate=0.2)
#
# # Print the model summary to verify the architecture
# cbapd_desync50_model.summary()

# Example of how to create the model for the synchronized ASCAD dataset
# No dropout is used for the synchronized dataset.
# cbapd_sync_model = create_cbapd_model(input_shape=(700, 1), dropout_rate=0.0)


# cbapd_sync_model.summary()

# 使用为ASCAD数据集指定的输入形状构建模型
# clrm_model = build_clrm_model(input_shape=(700, 1), num_classes=256)
...
class all(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if self.validation:
            ...
            Y_attack_valid = Y_attack_valid[:5000, :256]
...
def compute_adjustment(Y_profiling, tro):
    ...
    Y_profiling = np.argmax(Y_profiling[:, :classes], 1)
    label_freq = {}
...
def adjustment_loss(y_true, y_pred):
    ...
    if adjust_flag:
        y_pred = y_pred + 1 * adjustments_valid
...
if __name__ == '__main__':
    ...
    #
    model =  create_cbapd_model(input_shape=(700, 1), dropout_rate=0.0)
    model.compile(loss=adjustment_loss, optimizer='adam', metrics=None)
    model.summary()
    batch_size = 128
    # lr_manager = OneCycleLR(len(X_attack[:5000]), batch_size, learning_rate, end_percentage=0.2, scale_percentage=0.1,
    ...
    """开始训练"""
    history = model.fit(x=X_profiling[:50000], y=Y_profiling[:50000, :classes], batch_size=batch_size, verbose=2,
                        epochs=epoch,
                        callbacks=callback
                        )
    ...
    log = open('F:/result/cnn/a/0logit_curve_300.txt', mode='a',
               encoding='utf-8')
    ...
    def generate_image(text="程序已跑完", output_path="completed_matplotlib.png"):
        # 设置图像参数
        fig = plt.figure(figsize=(6, 3), dpi=150)  # 图像尺寸（宽600px，高300px）
