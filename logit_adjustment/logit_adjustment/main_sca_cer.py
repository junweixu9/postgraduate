...
class all(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if self.validation:
            ...
            logs['all_loss'] = float('inf')
            global best_weights
            global count
            X_attack_valid_metric, all_valid_plt_attack_metric, y_true = self.validation[0], self.validation[1], \
            self.validation[2]
            y_pred_valid_metric = self.model.predict(X_attack_valid_metric)
            # loss = cer_adjustment_loss(y_true,y_pred_valid_metric)
            # logs['all_loss'] = loss
            ...
            return logs['all_loss']


def compute_adjustment(Y_profiling, tro):
    """compute the base probabilities"""

    Y_profiling = np.argmax(Y_profiling[:, :256], 1)
    label_freq = {}
...
if __name__ == '__main__':
    ...
    log = open('F:/result/cer_logit/a/50000.txt', mode='a', encoding='utf-8')
    ...
    if sigma_hw == 0 and sigma_id == 0:
        ...
        """开始训练"""
        history = model.fit(x=X_profiling, y=Y_profiling[:, :256], batch_size=256, verbose=2, epochs=epoch,
                            callbacks=callback)
