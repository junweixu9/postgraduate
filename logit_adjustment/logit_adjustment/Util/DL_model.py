...
def pick_SOAT(data, leakage_model, length, metric, loss, learning_rate, model='MLP', model_size=64):
    if model == 'CNN':
        if data == 'ASCAD' and leakage_model == 'HW':
            batch_size = 256
            epoch = 50
            return cnn_architecture(length, metric, loss), batch_size, epoch  # cer_cnn_hw_not_ches cer_cnn_hw_ches
            ...
            batch_size = 128
            epoch = 50
            return ascad_r_hw_cnn(length, metric, loss), batch_size, epoch
        ...
        if data == 'ASCAD' and leakage_model == 'HW':
            batch_size = 32
            epoch = 10
            return ascad_f_hw_mlp(length, metric, loss, learning_rate), batch_size, epoch  # cer_mlp_zhang
        elif data == 'ASCAD' and leakage_model == 'ID':
            batch_size = 32
            epoch = 10
...
def cnn_architecture(length, metric, loss):
...
def cer_mlp_zhang(length, metric, loss):
...
def cer_cnn_hw_not_ches(length, metric, loss):
...
def cer_cnn_zhang(length, metric, loss):
    img_input = Input(shape=(length, 1))
    x = Conv1D(64, 2, kernel_initializer='he_uniform', activation='relu', padding='same')(img_input)
    x = AveragePooling1D(2, strides=2)(x)
    x = Conv1D(128, 2, kernel_initializer='he_uniform', activation='relu', padding='same')(x)
    x = AveragePooling1D(2, strides=2)(x)
    x = Conv1D(256, 2, kernel_initializer='he_uniform', activation='relu', padding='same')(x)
    x = AveragePooling1D(2, strides=2)(x)
    x = Conv1D(512, 2, kernel_initializer='he_uniform', activation='relu', padding='same')(x)
    x = AveragePooling1D(2, strides=2)(x)
    x = Conv1D(512, 2, kernel_initializer='he_uniform', activation='relu', padding='same')(x)
    x = AveragePooling1D(2, strides=2)(x)
    x = Flatten(name='flatten')(x)
...
def build_model_cnn(length, metric, loss):
    img_input = Input(shape=(length, 1))
    x = Conv1D(2, 25, kernel_initializer='he_uniform', activation='selu', padding='same')(img_input)
    x = AveragePooling1D(4, strides=4)(x)
    x = Flatten(name='flatten')(x)
...
def cer_cnn_hw_ches(length, metric, loss, l):
    img_input = Input(shape=(length, 1))
    ...
    x = Dense(9, name='predictions')(x)
    model = Model(img_input, x, name='ascad')
    optimizer = Adam(lr=5e-3)
    model.compile(loss=loss, optimizer=optimizer, metrics=metric)
    return model
