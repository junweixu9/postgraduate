...
def KD_distribution(yibufen_plt_attack, attack_byte, k_c):
    ...
    for i in range(num_plt_attack):
        # (num_plt_attack,256)
        k[i] = Utility.AES_Sbox()[np.bitwise_xor(range(256), int(yibufen_plt_attack[i, attack_byte]))]
...
def rank_compute(yibufen_attack_pred, yibufen_attack_plt, attack_byte, k_c, output_metric, leakage_model,
    ...
    for i in range(nb_traces):
        ...
        predicted_output = yibufen_attack_pred[
            i, Utility.AES_Sbox()[np.bitwise_xor(range(256), int(yibufen_attack_plt[i, attack_byte]))]]
        #  并进行对应密钥得分的相乘
