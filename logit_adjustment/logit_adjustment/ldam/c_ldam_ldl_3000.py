...
def compute_ldam(Y_profiling, max_m):
    ...
    m_list  = 1.0 / np.sqrt(np.sqrt(label_freq_array))
...
def adjustment_loss(y_true, y_pred):
    ...
    y_true_one_hot = tf.cast(tf.one_hot(y_true_index, depth=9), tf.float32)
    if adjust_flag:
        y_pred = y_pred - tf.multiply(ldam, y_true_one_hot)
...
if __name__ == '__main__':
    ...
    ldam = compute_ldam(Y_profiling, max_m)
    ldam = tf.cast(ldam, dtype=tf.float32)
    log = open('F:/result/ldam/c/3000.txt', mode='a',
               encoding='utf-8')
    print(file=log)
    print(file=log)
    """创建神经网络模型"""
