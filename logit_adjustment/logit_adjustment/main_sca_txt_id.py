...
from Util.SCA_util_id import perform_attacks
...
def kl_adjustment_loss(y_true, y_pred):
    ...
    if adjust_flag:
        y_pred = y_pred + 0.25 * adjustments
...
if __name__ == '__main__':
    ...
    log = open('F:/result/id/50000.txt', mode='a',
               encoding='utf-8')
    print(file=log)
    print(file=log)
    """创建神经网络模型"""
    ...
    if sigma_hw == 0 and sigma_id == 0:
        ...
        """开始训练"""
        history = model.fit(x=X_profiling, y=Y_profiling[:, :classes], batch_size=batch_size, verbose=2, epochs=epoch,
                            callbacks=callback
                            )
