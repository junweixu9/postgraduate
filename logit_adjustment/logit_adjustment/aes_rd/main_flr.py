...
def cnn_architecture(input_size=700, learning_rate=0.00001, classes=9):
    ...
    model.compile(loss=cer_adjustment_loss, optimizer='adam', metrics=None)
    model.summary()
    return model
...
if __name__ == '__main__':
    ...
    X_profiling = np.load("./profiling_traces_AES_RD.npy")
    Y_profiling = np.load("./profiling_labels_AES_RD.npy")
    X_attack = np.load("./attack_traces_AES_RD.npy")
    Y_attack = np.load("./attack_labels_AES_RD.npy")
    plt_profiling = np.load("./profiling_plaintext_AES_RD.npy")
    plt_attack = np.load("./attack_plaintext_AES_RD.npy")
