from data_load_distribution import read_data
...
def compute_adjustment(Y_profiling, tro):
    ...
    adjustments = np.expand_dims(adjustments, axis=0)
...
def adjustment_loss(y_true, y_pred):
    adjustments_gaosi = tf.multiply(adjustments, y_true[:, 10:])
    adjustments_gaosi = tf.cast(adjustments_gaosi, dtype=tf.float32)
    if adjust_flag:
        y_pred = y_pred + 1 * adjustments_gaosi
    y_pred = tf.nn.softmax(y_pred, 1)
    loss = tk.backend.categorical_crossentropy(y_true[:, :9], y_pred)
    return loss
...
if __name__ == '__main__':
    ...
    model_size = 128  # the size of the profiling model
    rank_logs = []
    all_rank_logs = []
    corr_logs = []
    all_corr_logs = []
    ...
    # adjustments_gaosi = tf.multiply(adjustments, gaosi)
    # adjustments_gaosi = tf.cast(adjustments_gaosi, dtype=tf.float32)
